...
class NCAFeedbackModel(NN.Module):
    def __init__(self, 
                 toroidal=False,
                 hidden_size=100, 
                 perception_size=3, 
    ...
    # batch_size x width x height -> batch_size x (1 + 1 + 1 + hidden_size) x width x height
    ...
    # batch_size x (1 + hidden_size) x width x height -> batch_size x (1 + 1 + 1 + hidden_size) x width x height
    def model_input_from_model_output(self, 
                                      model_output: torch.Tensor, 
                                      input_batch: torch.Tensor,
                                      errors: torch.Tensor) -> torch.Tensor:
        return torch.cat((
                input_batch.unsqueeze(-3), 
                errors.unsqueeze(-3),
                model_output), 
